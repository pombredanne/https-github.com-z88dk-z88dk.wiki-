...
Most of the classic targets support automatically configuring the heapsize and allow programs to allocate memory without requiring manual configuration. Typically the space between the last byte of the program and program's stack address can be used for the heap. The `AMALLOC` macros are available to configure the heap to be proportion of this space.

`-DAMALLOC` configures 3/4 of the space to be used as heap

`-DAMALLOC2` configures 1/2 of the space to be used as heap

`-DAMALLOC1` configures 1/4 of the space to be used as heap
...
The first declaration above reserves four bytes for the heap pointer that will be compiled statically as part of the final binary generated by the compiler.  
...
Must be called on program initialization to empty the heap if AMALLOC isn't being used.  An alternative is to zero the heap pointer with an assignment like "heap = 0L;"
