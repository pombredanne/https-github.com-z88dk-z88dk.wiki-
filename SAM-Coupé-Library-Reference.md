...
| misc.h | int | getpixel(int x, int y); |  | POINT command, return pixel pot colour from x,y or attribute from MODE1/2 | point() | graphics.h | 1 - High | SAM point(x,y) needs to return pot colour of pixel in high colour modes, not just binary set/not set |  
